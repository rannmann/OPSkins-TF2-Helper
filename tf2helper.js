...
// @match        https://opskins.com/*shop_browse*
...
(function() {
    'use strict';
    var tf2Schema = {};
    var itemList = getFromCache('tf2ItemMap');
    var effectIdMap = getFromCache('tf2EffectsMap'); // Keys are names, values are IDs.  The page only gives us the effect name, so this is seemingly backwards.
    var bptfApiKey = ''; // PUT YOUR API KEY HERE FOR BACKPACK TF OR THE JUICY DATAS ARE GONE
    var bptfData = {};
    loadBackpackTf(bptfApiKey);
    ...
    if (!itemList.hasOwnProperty('lastUpdatedTimestamp')) {
        console.log("Updating TF2 Schema");
        itemList.lastUpdatedTimestamp = Date.now();
        ...
        tf2Schema.result.items.forEach(function (item, index) {
            //if (item.item_class === "tf_wearable") {
                if (item.hasOwnProperty('used_by_classes')) {
                    itemList[item.item_name] = item.used_by_classes;
                } else {
                    itemList[item.item_name] = -1;
                }
            //}
        });

        // Get effect names and IDs
        effectIdMap = {};
        effectIdMap.lastUpdatedTimestamp = itemList.lastUpdatedTimestamp;
        tf2Schema.result.attribute_controlled_attached_particles.forEach(function (item, index) {
            effectIdMap[item.name] = item.id;
        });
        // Save in local storage
        localStorage.tf2ItemMap = JSON.stringify(itemList);
        saveToCache('tf2ItemMap', itemList);
        saveToCache('tf2EffectsMap', effectIdMap);
        console.log("Saved TF2 schema values");
    }
    ...
    function getFromCache(name) {
        ...
        if (localStorage.hasOwnProperty(name)) {
            ...
            if (res.hasOwnProperty('lastUpdatedTimestamp') && res.lastUpdatedTimestamp + 86400 * 1000 < Date.now()) {
                // reload it bruh.
    ...
    function saveToCache(name, obj) {
        localStorage[name] = JSON.stringify(obj);
    }

    function loadBackpackTf(apiKey) {
        if (localStorage.hasOwnProperty('backpackTfData')) {
            console.log("Found BackpackTF data in cache");
            bptfData = JSON.parse(localStorage.backpackTfData);
            // If this data is less than 14 days old, don't update it.
            if (bptfData.lastUpdatedTimestamp + 86400 * 1000 * 14 > Date.now()) {
                return;
            }
            console.log("BackpackTF data is too old, we'll update it momentarily.");
        }
        GM_xmlhttpRequest({
            method: "GET",
            url: "https://backpack.tf/api/IGetPrices/v4?key=" + apiKey,
            onload: function(response) {
                if (response.status === 200) {
                    bptfData = simplifyBackpackTf(JSON.parse(response.response));
                    bptfData.lastUpdatedTimestamp = Date.now();
                    localStorage.backpackTfData = JSON.stringify(bptfData);
                    console.log("Saved BackpackTF data to local storage");
                } else {
                    console.error("Unable to load data from BackpackTF: " + response.status);
                }
            }
        });
    }

    function simplifyBackpackTf(data) {
        var result = {};
        var effectNames = {};
        data = data.response;

        // Flip effects so keys are ints
        for (var name in effectIdMap) {
            effectNames[effectIdMap[name]] = name;
        }

        for (var itemName in data.items) {
            // Make sure it actually has a prices property
            if (!data.items[itemName].hasOwnProperty("prices")) {
                continue;
            }
            // Make sure it has an unusual quality
            if (!data.items[itemName].prices.hasOwnProperty("5")) {
                continue;
            }
            // Make sure there are actually tradable items.
            if (!data.items[itemName].prices["5"].hasOwnProperty("Tradable")) {
                continue;
            }

            // Just a sanity check... this should always be true.
            if (!data.items[itemName].prices["5"].Tradable.hasOwnProperty("Craftable")) {
                console.log("Found an unusual item that only has uncraftable versions: " + itemName);
                continue;
            }

            // For now, ignore uncraftable versions because we're lazy, and these shouldn't even exist if Valve was sane.

            for (var effectId in data.items[itemName].prices["5"].Tradable.Craftable) {
                // We know about this effect!
                if (effectNames.hasOwnProperty(effectId)) {
                    var value = parseFloat(data.items[itemName].prices["5"].Tradable.Craftable[effectId].value);
                    if (!result.hasOwnProperty(itemName)) {
                        result[itemName] = {};
                    }
                    if (data.items[itemName].prices["5"].Tradable.Craftable[effectId].currency === "keys") {
                        value = value * 1.90; // OPSkins conversion
                    } else if (data.items[itemName].prices["5"].Tradable.Craftable[effectId].currency === "metal") {
                        value = value * 0.06; // OPSkins/BPTF conversion
                    } else if (data.items[itemName].prices["5"].Tradable.Craftable[effectId].currency === "hat") {
                        value = value * 1.22 * 0.06; // BPTF conversion
                    } else if (data.items[itemName].prices["5"].Tradable.Craftable[effectId].currency === "earbuds") {
                        value = value * 4.29; // OPSkins conversion
                    }
                    // [item name][effect name] = value
                    result[itemName][effectNames[effectId]] = value;
                }
            }
        }
        return result;
    ...
    function applyToPage() {
        ...
        $('.app_440_2 .market-link:not(.tf2ized)').each(function() {
            ...
            if (itemList.hasOwnProperty(mn)) {
                // Append tf2 classes to item
                var str = '';
                if (itemList[mn] === -1) {
                    str = 'All';
                } else {
                    str = itemList[mn].join(', ');
                }
                $(this).parent().find('small.item-warning').append(' ' + str);
            }

            // Add mouseover text to particle effects
            ...
            if (desc) {
                ...
                if (effect && effects.hasOwnProperty(effect)) {
                    var title = '== Global Effect Information ==';
                    title = title + "\nRank:                       " + effects[effect].rank;
                    title = title + "\nPercentage:            top " + effects[effect].rank_pct + "%";
                    title = title + "\nAvg Value:              " + effects[effect].key_price + ' keys.';
                    title = title + "\nNumber Globally: " + effects[effect].exist;

                    if (bptfData.hasOwnProperty('lastUpdatedTimestamp')) {
                        if (bptfData.hasOwnProperty(mn)) {
                            if (bptfData[mn].hasOwnProperty(effect)) {
                                ...
                                if (itemPrice < bpPrice) {
                                    var diffPct = 100 * (itemPrice / bpPrice);
                                    var color = '';

                                    if (diffPct < 20) { // 20% of the suggested price: red
                                        color = "#4f1414";
                                    } else if (diffPct < 30) { // Orange
                                        color = "#4f3014";
                                    } else if (diffPct < 40) { // Yellow
                                        color = "#4f4e14";
                                    } else if (diffPct < 50) { // Yellow-green
                                        color = "#3d4f14";
                                    } else if (diffPct < 60) { // Green
                                        color = "#144f14";
                                    }
                                ...
                                console.log("No effect data for " + mn + " having " + effect);
                            }
                        } else {
                            console.log("No effect data for " + mn);
                        }
                    }
                    $(this).parent().find('.item-desc').find('small:nth-of-type(2)').prop('title', title).css('cursor', 'pointer');
